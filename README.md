...
- [環境需求與安裝步驟](#-環境需求與安裝步驟)
- [使用與執行指南](#-使用與執行指南)
  - [1. 預訓練模型權重下載](#1-預訓練模型權重下載-google-drive)
  - [2. 啟動輸入法主程式](#2-啟動輸入法主程式)
  - [3. 輕量測試模式 (免權重)](#3-輕量測試模式-免權重)
  - [4. 模型訓練與測試](#4-模型訓練與測試)
  - [5. 模型精確率評估](#5-模型精確率評估)
  - [6. 語料前處理與轉碼](#6-語料前處理與轉碼)
...
評估模型在不同最大序列長度（Sequence Length）配置下的準確率表現，測試集分為**中文**、**英文**與**中英混打**三種情境，並分別計算**子序列（Subsequence）**與**子字串（Substring）**之匹配精確度：

| 模型配置 / 長度 | 權重下載 | 中文 (output1)<br/>子序列 | 中文 (output1)<br/>子字串 | 英文 (output2)<br/>子序列 | 英文 (output2)<br/>子字串 | 混打 (output3)<br/>子序列 | 混打 (output3)<br/>子字串 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
| **len63 (v1)** | [下載](https://drive.google.com/file/d/1NSe-Zt0neS5Ta_h4hmvBeyqi4AuDR2aj/view?usp=sharing) | 86.97% | 37.20% | **70.89%** | **55.67%** | 52.14% | **42.88%** |
...
### 1. 預訓練模型權重下載 (Google Drive)

本專題提供三大主力指標表現最優異之預訓練權重檔，使用者可直接點擊連結至 Google Drive 下載，免去漫長訓練時間：

| 模型權重檔案名稱 | 核心強項與評估表現 | 下載連結 (Google Drive) |
| :--- | :--- | :---: |
| **`transcoder_len55_v1.pth`** | **中英混打表現最佳（日常推薦首選）**<br/>• 混打子序列：**55.35%**（全實驗最高）<br/>• 中文子字串：**37.62%**（全實驗最高）<br/>• 英文子序列：62.50% | [📥 Google Drive 下載](https://drive.google.com/file/d/1yZRFEFlNprgJroa0Lwz6EpdTqjpqut_-/view?usp=sharing) |
...
1. **放置模型權重**：
   確認已將欲使用的權重檔（例如 `transcoder_len55_v1.pth`）放置於 `model/` 資料夾下：
   ```text
   model/transcoder_len55_v1.pth
   ```
